"""CoWork Index API istemcisi + yerel durum -- Copilot Chat bağlayıcısı.

`connectors/claude_code/client.py` ile aynı deseni izler (kasıtlı olarak
küçük bir kod tekrarı var: iki bağlayıcının transcript biçimleri tamamen
farklı olsa da ağ/durum katmanı aynı; kaynak başına ayrı durum dosyası
gerektiği için tek bir paylaşımlı modüle çıkarmak şu an için gerekli değil).
"""
...
from connectors.copilot_chat.session_store import Exchange

STATE_DIR = Path(os.environ.get("COWORK_STATE_DIR", Path.home() / ".cowork_index"))
CONFIG_FILE = STATE_DIR / "copilot_chat_config.json"
STATE_FILE = STATE_DIR / "copilot_chat_state.json"
LOG_FILE = STATE_DIR / "copilot_chat.log"
CHUNK_SIZE = 20
...
def load_config() -> dict[str, Any]:
    ...
    cfg.setdefault("tool_name", "Copilot")
...
def select_changed(exchanges: list[Exchange], state: dict[str, str]) -> list[Exchange]:
    return [ex for ex in exchanges if state.get(ex.external_id) != exchange_fingerprint(ex)]
...
def post_exchanges(exchanges: list[Exchange], cfg: dict[str, Any], timeout: float = 60.0) -> list[dict]:
    ...
    for i in range(0, len(exchanges), CHUNK_SIZE):
        ...
        body = {
            ...
            "tool_name": cfg.get("tool_name", "Copilot"),
